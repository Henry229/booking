...
const Booking = ({clientRepository}) => {
  ...
  useEffect(() => {
    ...
    const settingState = (copyClients) => {
      ...
      return new Promise((resolve, reject) => {
        // const clone = {...states};
        // const cloneClient = {...clone, clients:{...copyClients}}
        // console.log('*** cloneClient : ', cloneClient);
        // clone.clients = cloneClient;
        // setStates(() => {return clone});
        // setStates(() =>{return clone});
        setStates(states =>{return {...states, clients:{...copyClients}}});
        console.log('*** states: ', states);
        // setStates(states => {return Object.assign({},states, {clients:{...copyClients}})});
        resolve(copyClients);
        // resolve(copyClients);
      })
    }
      ...
      return new Promise((resolve, reject) => {
        ...
        Object.keys(copyClients).map( key => {
          if ( copyClients[key].hasOwnProperty('bookDetail')) {
            updated = {...states};
            const upClient = [...updated.bookClient, {...copyClients[key]}]
            updated.bookClient = upClient;
            console.log('//// updated: ', updated);
            // setStates(() => {return updated});
          ...
          //   copyClients[key].bookDetail.map( book => {
          //     // console.log('====> xxxxx', copyClients[key],'/',copyClients[key].bookDetail[index],'/');
          //     if ( workingDate == book.bookDate) {
          //       console.log('@@@ matched: ', copyClients[key])
          //       setStates( prev => {
          //         const updated = {...prev};
          //         const updatedbook = updated.bookClient.concat(copyClients[key]);
